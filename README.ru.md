...
1. Нажмите `Открыть субтитры` (`Cmd+O`) или перетащите файл в область исходного файла.
2. Выберите папку выгрузки. Она запоминается между запусками.
3. Отметьте один или несколько форматов: `ASS`, `SRT`, `VTT` или `DOCX`.
4. Отметьте нужные роли в списке, если выгружаете отдельные файлы SRT по ролям.
5. Нажмите `Начать`. Долгую операцию можно прервать кнопкой `Отменить`.
